...
jq ".galaxy_url = \"$PORTAL_GALAXY_URL\" | .galaxy_api_key = \"$PORTAL_GALAXY_API_KEY\" | .deployment_repo_url = \"$CLOUD_DEPLOY_GIT_REPO\" | .announcement = \"$PORTAL_ANNOUNCEMENT\" | .tsi_portal_url = \"$TSI_PORTAL_URL\" | .aap_url = \"$AAP_URL\" | .enable_https = \"$ENABLE_HTTPS\"" /tmp/config.backup > $CONFIG_DIR/config.development.json
